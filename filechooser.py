...
from main import savePath_func
from main import SaveButtonChk
sys.path.insert(0,"/Users/macbook/Documents/python test/ScreenManager/main.py")
sys.path.append("/Users/macbook/Documents/python test/ScreenManager/main.py")
...
if __name__ == '__main__':
    ...
    BoxLayout:
        ...
        BoxLayout:
            ...
            Button:
                ...
                id: save
                disabled: True
                on_press: app.saveButton_fc()
                background_normal: ''
                background_color: rgba("#32cf70")
    ...
    def checkStr(Chkstr, self):
        global lm
        DataString = Chkstr.split(".")
        DataString = DataString[1].split("'")
        if DataString[0] == 'pdf' or DataString[0] == 'PDF' or DataString[0] == 'DOC' or DataString[0] == 'DOCX' or DataString[0] == 'doc' or DataString[0] == 'docx' or \
                DataString[0] == 'XLS' or DataString[0] == 'xls' or DataString[0] == 'TXT' or DataString[0] == 'txt':
            self.root.ids.save.disabled = False
            self.root.ids.textYm.text = ""
            lm = Chkstr
        else:
            global SaveButtonChk
            SaveButtonChk(False)
            self.root.ids.save.disabled = True
            self.root.ids.textYm.text = "Haraal idsen zuw utgaa oruul!"
        print(DataString[0])

    class FileChooserApp(App):

        def build(self):
            global savePath_func, checkStr
            v = root.ids.fc
            if len(sys.argv) > 1:
                v.path = sys.argv[1]

            if v.bind(selection=lambda *x: ("selection: %s" % x[1:])) != '':
                v.bind(selection=lambda *x: checkStr(("%s" % x[1:]), self))
            v.bind(path=lambda *x: pprint("path: %s" % x[1:]))

            return root

        def saveButton_fc(self):
            global SaveButtonChk
            savePath_func(lm)
            SaveButtonChk(True)
            sys.exit()
